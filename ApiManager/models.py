...
class BaseTable(models.Model):
    ...
    class Meta:#
        abstract = True # 将BaseTable定义为抽象类，不生成表
        verbose_name = "公共字段表"
        db_table = 'BaseTable'
...
class UserType(BaseTable):
    ...
    objects = UserTypeManager() # objects是模型models的属性，自定义一个UserTypeManager管理器类对象，通过管理器可以实现对数据的查询，用于模型对象与数据库交互
...
class ProjectInfo(BaseTable):
    class Meta:
        verbose_name = '项目信息' #verbose_name后台显示对对应的名称
...
class DebugTalk(BaseTable):
    ...
    belong_project = models.ForeignKey(ProjectInfo, on_delete=models.CASCADE)#ForeignKey引用外键，第一个参数是引用模型，第二个参数是外键删除后级联删除
