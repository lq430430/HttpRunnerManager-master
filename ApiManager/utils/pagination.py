...
def customer_pager(base_url, current_page, total_page, total_case):
    """
    返回可分页的html
    ...
    one = "<li><span>共'%s'条 </span></li>" % (total_case)  #添加用例总数
    pager_list.append(one)
...
def get_pager_info(Model, filter_query, url, id, per_items=12):
    ...
    elif url != '/api/env_list/' and url != '/api/debugtalk_list/': #url为用例列表页
    ...
    if total != 0:
        ...
        page_list = customer_pager(url, id, page_info.total_page,page_info.total_case)
