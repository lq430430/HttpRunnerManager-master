...
def add_case(request):
    ...
    account = request.session["now_account"]
    if request.is_ajax():
        testcase_info = json.loads(request.body.decode('utf-8'))#json.loads将json字符串转成dict
...
def add_config(request):
    ...
    account = request.session["now_account"]
    if request.is_ajax():
        testconfig_info = json.loads(request.body.decode('utf-8'))#将json字符串即string转换成dict
